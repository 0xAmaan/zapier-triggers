...
def create_event(event: schemas.EventCreate, db: Session = Depends(get_db)):
    ...
    db_event = models.Event(
        source=event.source, event_type=event.event_type, payload=event.payload
    )
    db.add(db_event)
    db.commit()
    db.refresh(db_event)
...
def get_inbox(
    ...
    limit: int = Query(
        default=50, ge=1, le=1000, description="Maximum number of events to return"
    ),
    ...
    return schemas.InboxResponse(events=event_details, total=len(event_details))
...
def acknowledge_event(event_id: str, db: Session = Depends(get_db)):
    ...
    return schemas.AcknowledgeResponse(
        event_id=event.id, status="acknowledged", acknowledged_at=event.acknowledged_at
    )
